...
\begin{proof}
  \case{
    $\Value(t,\sigma)=v$\\
    In this case, we have $\Simulate(t,\sigma,I,\phi)=\set{(v,I,\phi)}$.
    When we take $M$ to be the identity mapping, we obtain that
    $\Evaluate (M t,M\sigma,M I) = \Evaluate (t,\sigma,I) = v$, by definition of $\Evaluate$.
    }
  ...
  \case{
    ...
    $\Simulate'(\True,t,t',\sigma',I\oplus [i'],\phi\land\phi')$
    \case{
      $\Sat(\phi\land\phi')$\\
        \case{
          $\Value(t',\sigma')=v$\\
          In this case, $\Simulate'$ results in $\set{(v,I\oplus [i'],\phi\land\phi')}$.
          ...
          Which gives us exactly what we need to prove.\todo{something strange is going on with the input component. If $I$ contains something from the start, this does not work anymore....}
        }
    }
  }
\end{proof}
