...
The Task Oriented Programming paradigm focusses on modelling collaboration between users.
It provides a high level of abstraction in which programmers describe what work needs to be done and how.
From this description, the entire workflow system can be generated.

Programs in \TOP are called \emph{tasks}.
The basic elements of tasks are editors.
Using combinators, tasks can be combined into larger tasks.

Users can work together in a number of ways.
Tasks can be performed after each other or in parallel, and users can make choices between tasks.
...
This section introduces the main concepts of the language, together with some components essential for the next-step hint system.
