...
In this section, we intend to validate our approach by proving that our hints function is correct.
...
The theorems above make use of an evaluation function.
This function takes a task, state and list of inputs,
...
The definition of evaluate is given in \cref{def:evaluate}.
...
\begin{proof}[\cref{thm:soundhint}]
  \cref{thm:soundhint} follows from the premise of \refrule{R-Hints} and \cref{lem:soundsimulate} as follows.

  The premise of \refrule{R-Hints} gives us that for every pair $\tilde{\imath},\Phi)$ produced by $\Hints$,
...
Since the $\Hints$ function relies on the $\Simulate$ function,
we can make use of soundness and completeness of that function with respect to $\Evaluate$, when proving the lemma's above.


\subsection{Correctness of simulate}

Soundness and completeness for $\Simulate$ is defined below, together with the proofs for these lemma's.
...
\begin{tikzpicture}[
        ...
        \node (sk) [below of=ss,yshift=1.5cm] {$\tilde{t_k},\tilde{\sigma_k},\phi_k$};
        ...
        \node (sn) [below of=sss,yshift=1.5cm] {$\tilde{t_n},\tilde{\sigma_n},\phi_n$};
...
These lemma's and proofs are listed in the appendix.
